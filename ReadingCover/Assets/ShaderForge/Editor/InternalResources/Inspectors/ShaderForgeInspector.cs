    ...
    public class ShaderForgeInspector : Editor
    {
		...
		*/
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (GUILayout.Button("用 Shader Forge 打开"))
                {
            ...
            else
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    GUILayout.Label("找不到 Shader Forge 数据！", EditorStyles.miniLabel);
                    GUI.color = Color.white;
                }
                GUILayout.EndHorizontal();
                //GUILayout.Label( "Opening this will clear the shader", EditorStyles.miniLabel );
                //GUI.color = new Color( 1f, 0.8f, 0.8f );
                if (GUILayout.Button(new GUIContent("使用 Shader Forge 替换", SF_Styles.IconWarningSmall, "这将清除任何现有的着色器代码"), hasShaderForgeData ? "Button" : "MiniButton"))
                {
            ...
            if (SF_Tools.advancedInspector)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("打开着色器代码", btnStyle))
                    {
        ...
        public void DrawUnitysInspector()
        {
            ...
            {
                EditorGUILayout.LabelField("投射阴影", (!(bool)sutilHasShadowCasterPass.Invoke(null, new object[] { shader })) ? "no" : "yes", new GUILayoutOption[0]);
                EditorGUILayout.LabelField("渲染队列", ((int)sutilGetRenderQueue.Invoke(null, new object[] { shader })).ToString(CultureInfo.InvariantCulture), new GUILayoutOption[0]);
                EditorGUILayout.LabelField("细节层次（LOD）", ((int)sutilGetLOD.Invoke(null, new object[] { shader })).ToString(CultureInfo.InvariantCulture), new GUILayoutOption[0]);
                EditorGUILayout.LabelField("忽略投影器", (!(bool)sutilDoesIgnoreProjector.Invoke(null, new object[] { shader })) ? "no" : "yes", new GUILayoutOption[0]);
                string label;
                ...
                EditorGUILayout.LabelField("禁用批处理", label, new GUILayoutOption[0]);
        ...
        {
            GUILayout.Space(5f);
            GUILayout.Label("属性:", EditorStyles.boldLabel, new GUILayoutOption[0]);
        ...
        private void ShowCompiledCodeButton(Shader s)
        {
            ...
            EditorGUILayout.PrefixLabel("编译过的代码", EditorStyles.miniButton);
        ...
        private static void ShowSurfaceShaderButton(Shader s)
        {
            ...
            EditorGUILayout.BeginHorizontal(new GUILayoutOption[0]);
            EditorGUILayout.PrefixLabel("表面着色器", EditorStyles.miniButton);
            if (flag)
            {
