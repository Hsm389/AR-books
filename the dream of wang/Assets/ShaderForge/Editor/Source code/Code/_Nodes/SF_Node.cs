...
namespace ShaderForge
{
    ...
    public class SF_Node : ScriptableObject, IDependable<SF_Node>
    {
        ...
        public void RefreshValue(int ia, int ib)
        {
            ...
            {
                if (nc.required && nc.conType == ConType.cInput && !nc.IsConnected())
                { // Check only required inputs
                    texture.OnLostConnection();
                    return;
                }
            }
        ...
        public void OnRelease()
        {
            ...
            if (hover && stationary && !placingNew)
            { // If you released on the node without dragging
        ...
        public void DrawWindow()
        {
            ...
            else if (Event.current.type == EventType.ContextClick)
            {
                ...
                if (MouseOverNode(world: true))
                {
                    ...
                    menu.AddItem(new GUIContent("编辑注释"), false, ContextClick, "cmt_edit");
                    menu.AddItem(new GUIContent(nodeName + " 是做什么的？"), false, ContextClick, "doc_open");
            ...
            if (IsProperty() || codeNode)
            {
                ...
                if (selected || focusedField || mouseOver && !editor.screenshotInProgress)
                {
                    ...
                    // Right:
                    if (!IsGlobalProperty() && !codeNode)
                    { // Global ones *only* have internal names, display as main instead
            ...
            if (!SF_Settings.hierarchalNodeMove) // TODO: Snap toggle + make it work properly with hierarchal on
                foreach (SF_Node n in editor.nodes)
                {
                    ...
                    if (n.selected) // Don't snap to selected nodes
                        continue;
                    if (Mathf.Abs(n.rect.xMax - rect.xMax) < snapThreshold)
                    { // RIGHT SIDE SNAP
                        delta.x -= rect.xMax - n.rect.xMax;
                        rect.x = n.rect.xMax - rect.width;
                    }
                    else if (Mathf.Abs(n.rect.x - rect.x) < snapThreshold)
                    { // LEFT SIDE SNAP
                        delta.x -= rect.x - n.rect.x;
                        rect.x = n.rect.x;
                    }
                    else if (Mathf.Abs(n.rect.y - rect.y) < snapThreshold)
                    { // TOP SIDE SNAP
                        delta.y -= rect.y - n.rect.y;
                        rect.y = n.rect.y;
                    }
                    else if (Mathf.Abs(n.rect.center.x - rect.center.x) < snapThreshold)
                    ...
                    else if (Mathf.Abs(n.rect.center.y - rect.center.y) < snapThreshold)
        ...
        public string PreEvaluate()
        {
            ...
            else if (!varDefined && !neverDefineVariable)
            { // If it's not defined yet, define it! Append a new row
                DefineVariable();
            }
        ...
        public string Serialize(bool skipExternalLinks = false, bool useSuffixPrefix = false)
        {
            ...
            if (HasAnyInputConnected(skipExternalLinks))
            {
                ...
                int i = 0;
                foreach (SF_NodeConnector con in connectors)
                { // List connections, connected inputs only
